...
async function doh(req: Request): Promise<Response> {
  ...
  const isPost=req.method==='POST' && (req.headers.get('content-type')||'').startsWith('application/dns-message');
  if(!isGetDns && !isPost) return new Response('Bad Request',{status:400});
...
export async function GET(req: Request, ctx: { params: { token: string } }){ if(dohPath==='dns-query' || ctx.params.token!==dohPath) return new Response('Not Found',{status:404}); return doh(req); }
export async function POST(req: Request, ctx: { params: { token: string } }){ if(dohPath==='dns-query' || ctx.params.token!==dohPath) return new Response('Not Found',{status:404}); return doh(req); }
export async function OPTIONS(_: Request, ctx: { params: { token: string } }){ if(dohPath==='dns-query' || ctx.params.token!==dohPath) return new Response('Not Found',{status:404}); const h=new Headers(); h.set('access-control-allow-origin','*'); h.set('access-control-allow-methods','GET, POST, OPTIONS'); h.set('access-control-allow-headers','*'); return new Response(null,{status:204,headers:h}); }
